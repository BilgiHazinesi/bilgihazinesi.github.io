...
function login() {
    if(loginMode === 'teacher') {
        ...
        let sName = document.getElementById('studentLoginName').value.trim().toUpperCase();
        let sPass = document.getElementById('studentLoginPass').value.trim();
        if(students.includes(sName)) {
            if(studentPassObj[sName] === sPass) {
                loggedInStudent = sName;
                document.getElementById('loginOverlay').style.display = 'none';
                document.getElementById('appContainer').style.display = 'block';
...
function syncData() {
    ...
    const payload = JSON.stringify({ students, studentPass: studentPassObj, books, bookPages, records, settings, auth_password: teacherPassword });
...
function renderStudentPanel() {
    document.getElementById('studentNameDisplay').innerText = loggedInStudent;
    const listDiv = document.getElementById('studentMyBooksList');
    const treeDiv = document.getElementById('studentTreeArea');
    listDiv.innerHTML = "";
    
    let myRecs = records.filter(r => r.student === loggedInStudent);
    
    if(myRecs.length === 0) listDiv.innerHTML = "<p>Henüz kitap almadın.</p>";
    ...
    myRecs.forEach(r => {
        ...
        if(r.status === "Okuyor") {
            statusHtml = `<span style="color:#2563eb; font-weight:bold;">Okuyorsun</span>`;
        } else {
            statusHtml = `<span style="color:#10b981; font-weight:bold;">Teslim Ettin</span>`;
            if(!r.rating) {
                actionBtn = `<button class="btn-comment" onclick="studentRateBook(${r.id})">Değerlendir</button>`;
            } else {
                actionBtn = `<span style="font-size:0.8rem; color:#f59e0b;">⭐ ${r.rating}</span>`;
            }
        }
    ...
    let treeHtml = `
    <div class="tree-wrapper" style="height:350px;">
        <svg class="tree-svg-container" viewBox="0 0 400 500" preserveAspectRatio="xMidYMax slice">
                <defs>
                <linearGradient id="trunkGradS" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" style="stop-color:#4e342e;stop-opacity:1" /><stop offset="100%" style="stop-color:#8d6e63;stop-opacity:1" /></linearGradient>
                <radialGradient id="leafGradS" cx="50%" cy="50%" r="50%" fx="50%" fy="50%"><stop offset="0%" style="stop-color:#66bb6a;stop-opacity:1" /><stop offset="100%" style="stop-color:#2e7d32;stop-opacity:1" /></radialGradient>
            </defs>
            <path d="M180,500 L180,400 Q160,350 140,300 Q180,320 200,300 Q220,320 260,300 Q240,350 220,400 L220,500 Z" fill="url(#trunkGradS)" />
            <circle cx="200" cy="200" r="140" fill="url(#leafGradS)" opacity="0.9" /><circle cx="120" cy="250" r="80" fill="url(#leafGradS)" opacity="0.8" /><circle cx="280" cy="250" r="80" fill="url(#leafGradS)" opacity="0.8" /><circle cx="200" cy="100" r="90" fill="url(#leafGradS)" opacity="0.9" />
        </svg>
        <div id="studentFruits"></div>
    </div>
    <div style="text-align:center; margin-top:5px; font-weight:bold; color:var(--primary);">${completedRecs.length} Kitap Okudun!</div>
    `;
    treeDiv.innerHTML = treeHtml;
    
    let fruitsCont = document.getElementById('studentFruits');
...
function renderBookManager() {
    ...
    displayList.forEach((item) => {
        ...
        if(isEditMode) { contentHtml = `<div style="display:flex; flex-direction:column; gap:5px; width:100%;"><label style="font-size:0.75rem; color:var(--text-sub);">Kitap Adı:</label><input type="text" class="edit-input" id="edit-name-${item.originalIndex}" value="${item.name}"><div style="display:flex; gap:10px;"><div style="flex:1;"><label style="font-size:0.75rem; color:var(--text-sub);">Sayfa Sayısı:</label><input type="number" class="edit-input" id="edit-page-${item.originalIndex}" value="${item.pageCount}"></div><button class="btn-save-small" style="align-self:end; height:36px; margin-bottom:5px;" onclick="saveBookEdits(${item.originalIndex})">Kaydet</button></div></div>`; } 
            ...
            let details = "";
            if(item.status === 'out') { badge = `<span class="status-badge bg-red" style="color:#ef4444; font-weight:bold; font-size:0.8rem;">Dışarıda</span>`; item.activeList.forEach(r => { let isOverdue = checkOverdue(r.date); let warning = isOverdue ? `<span class="overdue-warning">⚠️ 15 Gün!</span>` : ""; let dateColor = isOverdue ? "#ef4444" : "inherit"; details += `<div style="font-size:0.85rem; margin-top:5px; display:flex; justify-content:space-between; align-items:center;"><span style="color:${dateColor}">🔴 <b>${r.student}</b> (${r.date}) ${warning}</span><button class="btn-delete" onclick="deleteRecord(${r.id})">Sil</button></div>`; }); } 
...
function genReport() { const s = document.getElementById('reportStudentInput').value.trim().toUpperCase(); if(!s) return; let myRecs = records.filter(r => r.student === s).sort((a,b) => a.id - b.id); let currentlyReading = myRecs.filter(r => r.status === "Okuyor"); let history = myRecs.filter(r => r.status === "İade Etti"); let totalP = 0; history.forEach(r => totalP += (parseInt(bookPages[r.book])||0)); let txt = `Sayın Velimiz,\n\n✨ "Her kitap keşfedilmeyi bekleyen ayrı bir dünyadır."\n\nÖğrencimiz *${s}*, bu dönem kütüphanemizden toplam *${myRecs.length}* kitap okuyarak okuma yolculuğunu zenginleştirmiştir.\nToplam Okunan Sayfa: *${totalP}*\n\n`; if (currentlyReading.length > 0) { txt += `🔴 *Şu An Okuduğu:* \n`; currentlyReading.forEach(r => { txt += `- ${r.book} (Alış: ${r.date})\n`; }); txt += `\n`; } if (history.length > 0) { txt += `📚 *Keşfettiği Dünyalar:* \n`; history.forEach((r, i) => { txt += `${i+1}. ${r.book} (✅ Okudu)\n`; }); } txt += `\nİlginiz ve desteğiniz için teşekkür ederiz.\nZeynal Öğretmen`; document.getElementById('reportOutput').innerText = txt; renderSpaceJourney(myRecs.length); }
function renderSpaceJourney(count) { const cont = document.getElementById('spaceJourney'); const svg = document.getElementById('journeySvg'); cont.querySelectorAll('.station-node, .astronaut').forEach(e => e.remove()); svg.innerHTML = ""; const points = [ {x: 15, y: 90}, {x: 85, y: 80}, {x: 15, y: 65}, {x: 85, y: 55}, {x: 15, y: 40}, {x: 85, y: 30}, {x: 15, y: 20}, {x: 85, y: 10}, {x: 50, y: 5} ]; const w = cont.offsetWidth; const h = cont.offsetHeight; let d = `M ${w*points[0].x/100} ${h*points[0].y/100}`; for(let i=1; i<points.length; i++) { d += ` L ${w*points[i].x/100} ${h*points[i].y/100}`; } let path = document.createElementNS("http://www.w3.org/2000/svg", "path"); path.setAttribute("d", d); path.setAttribute("fill", "none"); path.setAttribute("stroke", "rgba(255, 255, 255, 0.2)"); path.setAttribute("stroke-width", "4"); path.setAttribute("stroke-dasharray", "8,8"); svg.appendChild(path); points.forEach((p, i) => { let rank = RANKS[i]; let isReached = count >= rank.c; let node = document.createElement('div'); node.className = `station-node ${isReached ? 'active' : ''}`; node.style.left = p.x + "%"; node.style.top = p.y + "%"; node.innerHTML = `<div>${rank.t.split(' ')[0]}</div><div class="station-label">${rank.t}<br>${rank.c}</div>`; if(count >= rank.c && (i === RANKS.length-1 || count < RANKS[i+1].c)) { let astro = document.createElement('div'); astro.className = "astronaut"; astro.innerText = "👨‍🚀"; astro.style.left = p.x + "%"; astro.style.top = (p.y - 6) + "%"; cont.appendChild(astro); } cont.appendChild(node); }); }
