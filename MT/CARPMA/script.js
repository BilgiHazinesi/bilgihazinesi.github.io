document.addEventListener('DOMContentLoaded', () => {
    ...
    const showAnswerBtn = document.getElementById('show-answer-btn'); // YENİ
    ...
    missingDigitModeToggle.addEventListener('change', () => {
        ...
        if (isMissingMode) {
            prepareAnimation();
            fillGridWithResults(); // Gizli modda çözümü arkaplanda doldur
        } else {
            clearResults();
        }
    });
    ...
    function setupGrid(isMissingMode) {
        ...
        gridContainer.style.gridTemplateColumns = `repeat(${totalCols}, 45px)`;
    ...
    function addClickToToggle(cell) {
        ...
        cell.addEventListener('click', () => {
            ...
            if (cell.querySelector('input')) {
                ...
                input.dataset.correct = originalVal;
        ...
        if (n2Arr.length > 1) {
            ...
            for (let col = 0; col < maxCols; col++) {
                ...
                if (carry > 0) {
                    ...
                    // ÖNEMLİ: Toplama eldesini de kaydet
                    solutionMap.set(`add-carry-${col+1}`, carry);
                }
    ...
    // YENİ: CEVABI GÖSTER FONKSİYONU
    function showFullAnswer() {
        stopAnimation();
        fillGridWithResults();
        // Tüm adımlar bitmiş gibi ayarla
        currentStepIndex = animationQueue.length;
    }
    ...
    // Hafızadaki çözümü gride yazar
    function fillGridWithResults() {
        for (const [key, val] of solutionMap.entries()) {
            const parts = key.split('-');
            let selector = '';
            // Key formatları: partial-0-2, result-1, carry-0-1, add-carry-2
            ...
            else if (key.startsWith('carry')) selector = `[data-row='${parts[0]}-${parts[1]}'][data-col='${parts[2]}']`;
            else if (key.startsWith('add-carry')) selector = `[data-row='add-carry'][data-col='${parts[2]}']`;
